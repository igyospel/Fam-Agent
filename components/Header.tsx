...
const Header: React.FC<HeaderProps> = ({ isChatActive, onTriggerUpgrade, onHistory, onToggleSidebar }) => {
  return (
    <header className="h-16 flex items-center justify-between px-4 md:px-8 border-b border-gray-100 bg-white/80 backdrop-blur-md sticky top-0 z-30 transition-all w-full">
      ...
      <div className="flex items-center gap-2 md:gap-3">
        <button 
          ...
          className="flex items-center gap-2 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-3 py-1.5 md:px-4 md:py-2 rounded-full text-[10px] md:text-xs font-medium transition-all shadow-md shadow-orange-200 hover:shadow-lg hover:shadow-orange-200 active:scale-95"
        >
           <span className="w-4 h-4 rounded-full border border-white/40 flex items-center justify-center text-[8px] font-bold">$</span>
